...
// 댓글 목록 조회 및 페이징 표시
async function listAndDisplayPagination() {

    // 댓글 목록을 불러옴
    await list();
    // Pagination 객체를 사용하여 페이징을 HTML에 표시
    ...
    pagination.setRecordsPerPage(10); // 한 페이지에 보여줄 레코드 건수 설정
    pagination.setPagesPerPage(5); // 한 페이지에 보여줄 페이지 수 설정
    pagination.displayPagination(listAndDisplayPagination); // 페이징을 HTML에 표시하는 함수 호출
}

// 댓글 추가
async function addComment() {
  const url = 'http://localhost:9080/api/boards/comments';
  const currentBnum = document.getElementById('currentBnum').value; // 현재 게시물 번호
  const currentUname = document.getElementById('currentUname').value.slice(0, 30); // 현재 사용자 이름
  //const comment = document.getElementById('comment').value; // 댓글 내용

  const payload = { bnum: currentBnum,
...
// 댓글 수정
async function updateComment(bcommentId, updatedComment) {
  try {
    const response = await fetch(`/api/boards/comments/${bcommentId}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json'
      },
...
// 댓글 삭제
async function deleteComment(bcommentId) {
  try {
    const response = await fetch(`/api/boards/comments/${bcommentId}`, {
      method: 'DELETE'
    });
...
function list() {
    ...
    fetch(`/api/boards/comments?bnum=${currentBnum}&reqPage=${reqPage}&reqCnt=${reqCnt}`)
      ...
      .then(data => {
        if (data.header.rtcd === '00') {
          ...
          comments.forEach(comment => {
            ...
            li.dataset.replyNumber = comment.bcommentId; // 댓글 번호를 dataset에 저장
  ...
  if (event.target.classList.contains('cmodifyBtn')) {
    ...
    if (modifiedComment.trim() !== '') {
      ...
      saveBtn.addEventListener('click', () => {
        ...
        updateComment(bcommentId, updatedComment)
          .then(() => {
